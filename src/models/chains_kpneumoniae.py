"""
Classifier Chains for K. pneumoniae: XGBoost base estimator, modeling label dependencies.
...
Example:
    ...
    from src.config_kpneumoniae import DATA_ROOT, SPECIES, ANTIBIOTICS, OPTION_A_KPNEUMONIAE
    from src.models.chains_kpneumoniae import train_chain_kpneumoniae
    ...
    results_df = train_chain_kpneumoniae(X, dataset, split, 'temporal', OPTION_A_KPNEUMONIAE, 'option_a')
"""
...
from src.config_kpneumoniae import (
    ANTIBIOTICS,
    DATA_ROOT,
    OPTION_A_KPNEUMONIAE,
    RANDOM_SEED,
    REVERSE_ORDER_KPNEUMONIAE,
    SPECIES,
)
...
def train_chain_kpneumoniae(X, dataset, split, split_name, order, order_name):
    """
    Train and evaluate a Classifier Chain over a given K. pneumoniae antibiotic order.
    ...
    results_df['jaccard_score_zd1'] = joint_metrics['jaccard_score_zd1']
    results_df['exact_match_ratio'] = joint_metrics['exact_match_ratio']
    results_df['restricted_jaccard'] = joint_metrics['restricted_jaccard']
    ...
    results_df.to_csv(output_dir / f'chains_kpneumoniae_{order_name}_{split_name}.csv', index=False)
...
if __name__ == '__main__':
    ...
    ORDERS = [
        (OPTION_A_KPNEUMONIAE, 'option_a'),
        (REVERSE_ORDER_KPNEUMONIAE, 'reverse_order'),
    ]
    ...
    for order, order_name in ORDERS:
        results_df = train_chain_kpneumoniae(X, dataset, temporal_split_result, 'temporal', order, order_name)
    ...
    for seed in range(10):
        ...
        for order, order_name in ORDERS:
            results_df = train_chain_kpneumoniae(X, dataset, split, f'random_seed{seed}', order, order_name)
            results_df['seed'] = seed
            random_results_dfs.append(results_df)

    random_results_df = pd.concat(random_results_dfs, ignore_index=True)
    random_results_df.to_csv(output_dir / 'chains_kpneumoniae_random_seeds.csv', index=False)
    ...
    joint_cols = [
        'hamming_loss', 'jaccard_score', 'jaccard_score_zd1',
        'exact_match_ratio', 'restricted_jaccard', 'all_zero_frac', 'chain_baseline_agreement',
    ]
    ...
    summary_df.to_csv(output_dir / 'chains_kpneumoniae_random_summary.csv', index=False)
