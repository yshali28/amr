"""
Classifier Chains for S. aureus: XGBoost base estimator, modeling label dependencies.
...
Example:
    ...
    from src.config_saureus import DATA_ROOT, SPECIES, ANTIBIOTICS, OPTION_A_SAUREUS
    from src.models.chains_saureus import train_chain_saureus
    ...
    results_df = train_chain_saureus(X, dataset, split, 'temporal', OPTION_A_SAUREUS, 'option_a')
"""
...
from src.config_saureus import (
    ANTIBIOTICS,
    DATA_ROOT,
    MISSEEDED_CIPROFLOXACIN,
    MISSEEDED_CIPROFLOXACIN_PENICILLIN_SECOND,
    MISSEEDED_OXACILLIN,
    OPTION_A_SAUREUS,
    RANDOM_SEED,
    SPECIES,
...
def train_chain_saureus(X, dataset, split, split_name, order, order_name):
    """
    Train and evaluate a Classifier Chain over a given S. aureus antibiotic order.
    ...
    results_df.to_csv(output_dir / f'chains_saureus_{order_name}_{split_name}.csv', index=False)
...
if __name__ == '__main__':
    ...
    ORDERS = [
        (OPTION_A_SAUREUS, 'option_a'),
        (MISSEEDED_OXACILLIN, 'misseeded_oxacillin'),
        (MISSEEDED_CIPROFLOXACIN, 'misseeded_ciprofloxacin'),
        (MISSEEDED_CIPROFLOXACIN_PENICILLIN_SECOND, 'misseeded_cipro_pen2'),
    ]
    ...
    for order, order_name in ORDERS:
        results_df = train_chain_saureus(X, dataset, temporal_split_result, 'temporal', order, order_name)
    ...
    for seed in range(10):
        ...
        for order, order_name in ORDERS:
            results_df = train_chain_saureus(X, dataset, split, f'random_seed{seed}', order, order_name)
            results_df['seed'] = seed
            random_results_dfs.append(results_df)

    random_results_df = pd.concat(random_results_dfs, ignore_index=True)
    random_results_df.to_csv(output_dir / 'chains_saureus_random_seeds.csv', index=False)
    ...
    per_seed_joint = random_results_df.drop_duplicates(subset=['order_name', 'seed'])[
        ['order_name', 'hamming_loss', 'jaccard_score', 'all_zero_frac']
    ]
    joint_summary = (
        per_seed_joint.groupby('order_name')[['hamming_loss', 'jaccard_score', 'all_zero_frac']]
        .agg(['mean', 'std'])
    )
    ...
    summary_df.to_csv(output_dir / 'chains_saureus_random_summary.csv', index=False)

    # final comparison: all_zero_frac per ordering, both splits
    temporal_zero_fracs = pd.DataFrame([
        {
            'order_name': results_df['order_name'].iloc[0],
            'split': 'temporal',
            'all_zero_frac': results_df['all_zero_frac'].iloc[0],
        }
        for results_df in temporal_results_dfs
    ])
    random_zero_fracs = (
        joint_summary[['order_name', 'all_zero_frac_mean']]
        .rename(columns={'all_zero_frac_mean': 'all_zero_frac'})
        .assign(split='random_mean')
    )
    comparison_df = pd.concat([temporal_zero_fracs, random_zero_fracs], ignore_index=True)

    print("\nall_zero_frac comparison (temporal vs. mean-across-seeds random), by ordering:")
    print(comparison_df)
