"""
Binary Relevance baseline for K. pneumoniae: one independent XGBoost model per antibiotic.

Example:
    from maldi_learn.driams import load_driams_dataset
    from src.data.features import to_feature_matrix
    from src.data.split import random_split
    from src.config_kpneumoniae import DATA_ROOT, SPECIES, ANTIBIOTICS
    from src.models.baseline_kpneumoniae import train_baseline_kpneumoniae
    ...
    results_df = train_baseline_kpneumoniae(X, dataset, split, 'random')
"""
...
from src.config_kpneumoniae import ANTIBIOTICS, RANDOM_SEED
from src.evaluation.metrics import multilabel_metrics

PROJECT_ROOT = Path(__file__).resolve().parents[2]


def train_baseline_kpneumoniae(X, dataset, split, split_name):
    """
    Train and evaluate a per-antibiotic XGBoost baseline for K. pneumoniae.

    Example:
        results_df = train_baseline_kpneumoniae(X, dataset, split, 'temporal')
    """
    ...
    results_df.to_csv(output_dir / f'baseline_kpneumoniae_{split_name}.csv', index=False)
...
if __name__ == '__main__':
    ...
    from src.config_kpneumoniae import DATA_ROOT, SPECIES
    from src.data.features import to_feature_matrix
    ...
    temporal_results_df = train_baseline_kpneumoniae(X, dataset, temporal_split_result, 'temporal')
    ...
    for seed in range(10):
        ...
        results_df = train_baseline_kpneumoniae(X, dataset, split, f'random_seed{seed}')
        results_df['seed'] = seed
        random_results_dfs.append(results_df)

    random_results_df = pd.concat(random_results_dfs, ignore_index=True)
    random_results_df.to_csv(output_dir / 'baseline_kpneumoniae_random_seeds.csv', index=False)
    ...
    per_antibiotic_summary.to_csv(output_dir / 'baseline_kpneumoniae_random_summary.csv', index=False)
